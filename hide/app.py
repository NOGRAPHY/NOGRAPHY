...
def lambda_handler(event, context):
    ...
    if len(secret) * ENCODING_DECODING_BASE > len(placeholder):
        ...
        encoded_secret = [int(c, 2) for c in encoder.encode(
            secret, ENCODING_DECODING_BASE)]
...
def fit_text(img, letters_and_fonts, color, margin):
    ...
    for t, w, h in pieces:
        ...
        for letter, font in t:
            font = ImageFont.truetype('./assets/'+str(font)+'.ttf', FONT_SIZE)
